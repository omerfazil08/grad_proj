# evolution_colab_phase35.py
# Phase 3.5: Smart Context-Aware Mutation
# Adjusts mutation rate based on proximity to solution.
...
# ... [Keep BitwiseConfig, Logic Gates, HSS Helpers, Helpers same as Phase 3.4] ...
# (Copy lines 1-130 from evolution_colab_phase34.py here)
class BitwiseConfig:
...
def evolve_bitwise(num_inputs, num_outputs, inputs_list, targets_list, cfg):
    ...
    print(f"Bitwise v3.5: Context-Aware Mutation Strategy.")
    ...
    # Specialists
    specialists = [None] * num_outputs
    specialist_scores = [-1] * num_outputs
    ...
    try:
        for gen in range(cfg.generations):
            ...
            # Update Specialists
            for idx, (ind, bkd) in enumerate(zip(population, breakdowns)):
                for col_i, score in enumerate(bkd):
                    if score > specialist_scores[col_i]:
                        specialist_scores[col_i] = score
                        specialists[col_i] = ind
            ...
            # Determine closest unsolved target distance
            min_dist_to_solve = float('inf')
            for i in range(num_outputs):
                if not solved_mask[i]:
                    dist = max_score_per_col - specialist_scores[i]
                    if dist < min_dist_to_solve: min_dist_to_solve = dist
            
            # Logic:
            # Dist < 50 (Very close) -> Mutation 0.02 (Fine tuning)
            # Dist > 100 (Far) -> Mutation 0.20 (Exploration)
            if min_dist_to_solve < 50:
                current_mut_rate = 0.02
            elif min_dist_to_solve < 150:
                current_mut_rate = 0.05
            else:
                current_mut_rate = 0.15

            # Stagnation Override
            if (gen - last_avg_improvement) > 100:
                current_mut_rate = 0.35 # Shock
                if gen % 50 == 0: print(f"⚠️ Stagnation. Shocking.")
            elif mu > best_avg_so_far:
                best_avg_so_far = mu
                last_avg_improvement = gen
            ...
            for s in specialists:
                if s is not None: new_pop.append(s)
