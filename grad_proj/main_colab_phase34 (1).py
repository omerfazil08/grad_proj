...
import evolution_colab_phase34 # The new engine
...
def run_5bit_adder_bitwise():
    ...
    cfg = evolution_colab_phase34.BitwiseConfig(
        num_gates=200, 
        pop_size=3000,
        generations=6000, 
        elitism=150,
    ...
    best_ind_raw, best_bkd, hof, history = evolution_colab_phase34.evolve_bitwise(
        n_in, n_out, inputs, targets, cfg
    )
    
    print("Converting genome for export...")
    best_ind_str = evolution_colab_phase34.convert_to_string_format(best_ind_raw, n_in)
