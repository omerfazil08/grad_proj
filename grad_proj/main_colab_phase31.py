# main_bitwise.py
# High-Speed Bitwise Driver for 5-bit+ Circuits

import matplotlib.pyplot as plt
import numpy as np
import sys
import vhdl_generator
import evolution_colab_phase31  # The new engine
...
# ... (Helper functions like get_user_target same as before) ...
def get_user_target():
    try:
...
def run_5bit_adder_bitwise():
    ...
    # Use BitwiseConfig
    cfg = evolution_colab_phase31.BitwiseConfig(
        num_gates=60, 
        pop_size=3000,        # Bigger pop because it's fast now!
        generations=5000, 
        elitism=50,
        tournament_k=10,
    ...
    print("🚀 Starting High-Speed Bitwise Evolution...")
    
    best_ind_raw, best_bkd, hof, history = evolution_colab_phase31.evolve_bitwise(
        n_in, n_out, inputs, targets, cfg
    )
    
    # Convert internal int-representation to string-representation for VHDL
    print("Converting genome for export...")
    best_ind_str = evolution_colab_phase31.convert_to_string_format(best_ind_raw, n_in)
    
    print(f"Final Scores: {best_bkd}")
    
    # VHDL
    vhdl = vhdl_generator.generate_vhdl_code(best_ind_str, n_in, n_out)
