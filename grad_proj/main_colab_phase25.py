...
# Updated with 5 Menu Options & Plot Saving
...
try:
    ...
    print("Warning: Simplifier missing. Install 'sympy' to enable.")
...
def run_2bit_adder():
    # 4 inputs (A[1:0], B[1:0]), 3 outputs (Sum[1:0], Cout)
    n_in = 4; n_out = 3
    inputs = []; targets = [[], [], []] # S0, S1, Cout
    for i in range(16):
        a1=(i>>3)&1; a0=(i>>2)&1; b1=(i>>1)&1; b0=(i>>0)&1
        inputs.append((a1, a0, b1, b0))
        val = ((a1<<1)|a0) + ((b1<<1)|b0)
        targets[0].append((val>>0)&1) # S0
        targets[1].append((val>>1)&1) # S1
        targets[2].append((val>>2)&1) # Cout
...
def run_3bit_adder():
    # 6 inputs (A[2:0], B[2:0]), 4 outputs (Sum[2:0], Cout)
    n_in = 6; n_out = 4
    inputs = []; targets = [[], [], [], []]
    for i in range(64):
    ...
    cfg = ColabConfig(
        num_gates=32, pop_size=1500, generations=3000, elitism=25, tournament_k=7,
        base_mut=0.05, min_mut=0.005, p_choose_primitive=0.60, log_every=50,
        record_history=True, seed=42, size_penalty_lambda=0.0, parallel=True
    )
...
def run_5bit_adder_complex():
    # 10 inputs (A[4:0], B[4:0]), 6 outputs (Sum[4:0], Cout)
    # The "10 in 6 out" benchmark
    n_in = 10; n_out = 6
    print(f"Generatng Truth Table for {n_in} inputs (1024 rows)...")
    
    inputs = []
    targets = [[] for _ in range(n_out)]
    
    for i in range(1024):
        # Extract A (high 5 bits) and B (low 5 bits)
        val_a = (i >> 5) & 0x1F
        val_b = i & 0x1F
        
        # Build tuple for input
    ...
    cfg = ColabConfig(
        num_gates=50, # Needs significant logic depth
        pop_size=2000, # Large population for large search space
        generations=5000, 
        elitism=30,
        tournament_k=8,
        base_mut=0.05, min_mut=0.005, 
        p_choose_primitive=0.60, # Macros help here
        log_every=50,
        record_history=True, 
        seed=42, 
    ...
    run_and_visualize(n_in, n_out, inputs, targets, cfg, "5-bit Adder (10-in 6-out)")
