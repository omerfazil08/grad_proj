# evolution_colab_phase34.py
# Phase 3.4: Bitwise Engine + Hardware Locking + Specialist Elitism
# Fixes: Prevents loss of partial solutions for hard outputs (e.g. 992/1024)
...
def evolve_bitwise(num_inputs, num_outputs, inputs_list, targets_list, cfg):
    ...
    print(f"Bitwise v3.4: Locking + Specialist Elitism + Soft Hyper-Mutation.")
    
    population = init_population(num_inputs, cfg)
    solved_mask = [False] * num_outputs
    locked_gates = {} 
    
    # Specialists: Store the best individual for EACH output
    ...
    try:
        for gen in range(cfg.generations):
            ...
            # Update Specialists (The crucial fix)
            for idx, (ind, bkd) in enumerate(zip(population, breakdowns)):
                for col_i, score in enumerate(bkd):
                    if score > specialist_scores[col_i]:
                        ...
                        specialists[col_i] = ind # Copy reference
            ...
            is_stagnant = (gen - last_avg_improvement) > 100 # Increased patience
            current_mut_rate = cfg.base_mut * (1 - gen/cfg.generations) + cfg.min_mut
            
            if is_stagnant:
                current_mut_rate = 0.35 # Slightly less violent
                if gen % 20 == 0: 
                    print(f"⚠️ Stagnation (Last impr: {last_avg_improvement}). Soft Hyper-Mutation.")
